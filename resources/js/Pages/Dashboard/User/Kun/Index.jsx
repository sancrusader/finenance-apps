...
export default function NotaExtract() {
  ...
  const captureImage = () => {
    ...
    if (image) {
      ...
      img.onload = () => {
        ...
        context.drawImage(img, x, y, 720 * scale, 1280 * scale, 0, 0, canvas.width, canvas.height)
  ...
  const submitForm = (e) => {
    ...
    if (imageSrc?.blob) {
      formDataToSend.append("images", imageSrc.blob, "receipt.jpg");
    }
  
    axios
      .post(route("nota.database"), formDataToSend, {
        headers: {
      ...
      .then((response) => {
        ...
        // Reset form
        setFormData({
          name: user.name, // Tetap isi dengan nama user
          tanggal: "",
          nama_toko: "",
          deskripsi: "",
          nominal: "",
  ...
  return (
    <UserLayout>
      ...
      <div className="max-w-md mx-auto space-y-4 p-4">
        ...
        <Button onClick={submitForm} disabled={isLoading} className="w-full mt-4">
          {isLoading ? (
            <>
              <Loader className="mr-2 h-4 w-4 animate-spin" />
              Mengirim Data...
